// Package vmess is a fork of sing-box's protocol/vmess inbound
// (github.com/sagernet/sing-box@v1.14.0, protocol/vmess/inbound.go) with the
...
type Inbound struct {
	...
	service    *vmess.Service[*userkey.Key]
...
func NewInbound(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.VMessInboundOptions) (adapter.Inbound, error) {
	...
	service := vmess.NewService[*userkey.Key](adapter.NewUpstreamContextHandler(inbound.newConnectionEx, inbound.newPacketConnectionEx), serviceOptions...)
...
// already passed the handshake are unaffected.
func (h *Inbound) UpdateUsers(users []option.VMessUser) error {
	return h.service.UpdateUsers(userkey.Build(common.Map(users, func(it option.VMessUser) string {
		return it.Name
