...
describe("buildIntegrationsPatch", () => {
  it("omits a key that was never touched (no draft, not cleared)", () => {
    ...
    expect(body).toEqual({});
    expect("kirbot_url" in body).toBe(false);
  });

  it("a cleared key wins over a stray draft value for the same field", () => {
    const body = buildIntegrationsPatch(
      fields,
      { kirbot_url: "http://leftover-draft.example" },
      new Set(["kirbot_url"])
    );
    expect(body.kirbot_url).toBeNull();
  });

  it("a non-empty draft for an uncleared text field is sent as-is", () => {
    const body = buildIntegrationsPatch(fields, { kirbot_url: "http://new.example" }, new Set());
    expect(body).toEqual({ kirbot_url: "http://new.example" });
  });

  it("a draft that trims to empty is treated as untouched, not cleared", () => {
    const body = buildIntegrationsPatch(fields, { kirbot_url: "   " }, new Set());
    expect(body).toEqual({});
  });
  ...
  it("accepts clearedKeys as a plain array as well as a Set", () => {
    const body = buildIntegrationsPatch(fields, {}, ["kirbot_url"]);
    expect(body).toEqual({ kirbot_url: null });
  });

  it("builds a mixed patch: one cleared, one changed, the rest omitted", () => {
    const body = buildIntegrationsPatch(
      fields,
      { kirbot_url: "http://new.example", telegram_api_token: "unused-because-cleared" },
      new Set(["telegram_api_token"])
    );
    expect(body).toEqual({
      kirbot_url: "http://new.example",
      telegram_api_token: null,
    });
    expect(Object.keys(body).sort()).toEqual(["kirbot_url", "telegram_api_token"]);
  });
});
